...
import { close, setPortrait, setWindowSize } from '../store/app/actions'
...
class Footer extends React.Component<ConnectedProps<typeof connector>> {
    ...
    handleResize = () => {
        ...
        if (
            Math.abs(width - innerWidth) > BaseBoardSize / 2 ||
            Math.abs(height - innerHeight) > BaseBoardSize / 2
        ) {
            ...
            boardRect &&
                updateBoardState(
                    tur.updateCellsAndTowersPosition(
                        boardAndTowers,
                        boardOptions,
                        boardRect
                    )
