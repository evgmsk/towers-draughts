...
function* workerNewGameVSPlayer() {
    ...
    if (playerColor === 'random') {
        color = Math.random() < 0.5 ? PieceColor.white : PieceColor.black
    }
    ...
    yield put({ type: BOA.REVERSE_BOARD, payload: color === PieceColor.black })
    const gamePayload: Partial<IGameState> = {
        moveOrder: { pieceOrder: PieceColor.white, playerTurn: 'White' },
        black,
        white,
        gameStarted: true,
        playerColor: color,
    ...
    yield put({
        ...
        payload: { blackClock: clock, whiteClock: clock },
    })
...
function* workerNewGameVsPC(action: GameActionTypes) {
    const {
    ...
    yield put({
        ...
        payload: { ...InitialTowersState, moves },
    })
    let color: PieceColor = playerColor
    if (playerColor === 'random') {
        color = Math.random() < 0.5 ? PieceColor.white : PieceColor.black
    ...
    const moveOrder: IMoveOrder = {
        ...
        playerTurn: color === PieceColor.white ? name : rival.name,
    }
    yield put({ type: BOA.REVERSE_BOARD, payload: color === PieceColor.black })
    const white =
        color === PieceColor.white
            ? { name, rating }
            : ({ name: rival.name } as IPlayer)
    const black =
        color === PieceColor.white
            ? { name: rival.name, rivalLevel }
            : ({ name, rating } as IPlayer)
    const gamePayload: Partial<IGameState> = {
        moveOrder,
        playerColor: color,
        white,
    ...
    yield put({
        ...
        payload: window.innerWidth / window.innerHeight < 1.3,
    })
    yield put({ type: GOA.FINISH_GAME_SETUP, payload: true })
...
function* checkDraw(payload: IMoveToMake) {
    const {
        ...
        game: { ineffectiveMoves },
    } = yield select()
    if (
        (gameVariant !== 'international' && ineffectiveMoves > 30) ||
...
function* workerPlayerClockAfterMove(payload: IMoveToMake) {
    ...
    if (gameConfirmed) {
        if (payload.moveOrder.pieceOrder === PieceColor.white) {
            const payload = {
                ...blackClock,
                timeToGame: blackClock.timeToGame + blackClock.adds,
            }
            ...
            const payload = {
                ...whiteClock,
                timeToGame: whiteClock.timeToGame + whiteClock.adds,
            }
        ...
        if (payload.moveOrder.pieceOrder === PieceColor.black) {
...
function* workerGameEnd(action: GMA) {
    ...
    const draw =
        action.payload === 'drawByAgreement' || action.payload === 'drawByRules'
    ...
    if (action.payload === 'abandonedByWhite') {
        ...
        winner = !blackClock.timeToGame ? PieceColor.white : PieceColor.black
    } else {
...
function* resolveEndGame(
    ...
    const PC = playerColor === PieceColor.white ? black.name : white.name
    const gameResult: Partial<IGameResult> & {
        PC: string
    ...
    const analysisPayload = {
        ...
        currentMove: { index: history.length - 1, move: history.slice(-1)[0] },
    }
    yield put({
