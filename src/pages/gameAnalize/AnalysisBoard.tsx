...
import {IGameBoard,  ISeekerProps} from "../../store/models";
import {updateBoardSize, } from "../../store/board-towers/actions";
...
export class GameBoard extends React.Component<GameAnalyzeProps, AnalysisBoardState> {
    ...
    componentDidMount() {
        ...
        setTimeout(() => {
            const ref = this.boardRef.current
    ...
    componentDidUpdate(prevProps: GameAnalyzeProps) {
        const {
            ...
            boardOptions,
            boardAndTowers: {towers, cellSize, cellsMap},
            windowSize
        } = this.props
    ...
    //         boardAndTowers: {towers},
    //     } = this.props
    //     const mandatoryMoves = mmr.lookForMandatoryMoves(pieceOrder, towers)
    ...
    //     const {boardAndTowers, updateBoardState} = this.props
    ...
    //     const currentPosition = copyObj(boardAndTowers.towers)
    ...
    //             towers = tur.updateTowersOnMandatoryMoveStep(from, cellKey, boardAndTowers, tP, true)
    ...
    //             : mmr.kingTowerAvailableMoves(key, currentPosition!, cellsMap!)
    //     }
    ...
    render() {
        ...
        const {towers, mandatoryMoves, mandatoryMoveStep, lastMoveSquares} = boardAndTowers
        const boardProps = {
            boardOptions,
        ...
        const mandatoryTowers = (mandatoryMoves || []).map(m => m.move[mandatoryMoveStep || 0])
