...
const GameEndPopup: React.FC<ConnectedProps<typeof connestor>> = (props) => {
    ...
    const notAbandonByPlayer = !(
        reason &&
        reason.startsWith('abandon') &&
        reason.includes(playerColor as string)
    )
    let message
    if (!winner) {
        ...
        <section className="game-end-popup">
            <header className={`popup-header ${win ? 'win-bg' : 'lose-bg'}`}>
                ...
                <span
                    ...
                    onClick={() => setGameMode('isPreparing')}
                >
                    cancel
                </span>
            </header>
            ...
            <div
                className={`popup-footer ${win ? 'win-bg' : 'lose-bg'}`}
                onClick={handleClick}
            >
    ...
    return <>{gameOver && notAbandonByPlayer ? popupHTML : null}</>
}
