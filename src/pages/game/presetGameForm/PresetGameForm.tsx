...
const RivalVariant: React.FC<ConnectedProps<typeof gameTypeConnector>> = (
    props
) => {
    ...
    const buttonClass = (value: GameType) =>
        `opponent-variant__b ${gameType === value ? 'actual' : ''}`
    ...
    return (
        <div className="timing-wrapper" onClick={handleSelectTime}>
            {StandardTiming.map((t: number[], i: number) => {
                const Class =
                    t[0] === timeToGame && t[1] === adds
                        ? 'timing-menu-item actual'
                        : 'timing-menu-item'
                return (
                    <button
                        title={
                            'time limit: minuts / add per rivalMove: seconds'
                        }
                        key={i}
                        className={Class}
                        type="button"
    ...
    return (
        <div className="level-wrapper">
            {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((i: number) => {
                const Class =
                    level === i ? 'level-menu-item actual' : 'level-menu-item'
                return (
                    <button
...
const gameVariantConnector = connect(
    gameVariantMapState,
    gameVariantMapDispatch
)
...
const RivalTypeSelector = (
    props: ConnectedProps<typeof rivalTypeConnector>
) => {
...
const customTimingConnector = connect(
    customTimingMapState,
    customTimingMapDispatch
)

type CTP = ConnectedProps<typeof customTimingConnector>

export class CustomTimingPreset extends React.Component<
    CTP,
    { limit: number; adds: number; open: boolean }
> {
    ...
    componentDidUpdate(prev: CTP) {
        ...
        const ST = StandardTiming.map((t) => t.join('/')).includes(
            `${timeToGame}/${adds}`
        )
    ...
    handleClick: React.MouseEventHandler<HTMLDivElement> = (
        ...
        if (
            target.classList.contains('custom-timing__values') &&
            this.state.open
        ) {
            this.setState({ open: false })
        }
    ...
    render() {
        ...
        return (
            <div
                className={`custom-timing ${open ? 'open' : 'close'}`}
