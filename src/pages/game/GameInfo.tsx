...
const PlayerInfo = (props: { color: PieceColor }) => {
    const { color } = props
    const { onlineStatus, name, rating } = useSelector(
        (state: IRootState) => state.game[color]
    )
    const rt = useSelector((state: IRootState) => state.gameOptions.rivalType)
    const VSComputer = rt === 'PC'
    const Color = `${(color as string)[0].toUpperCase()}${(
        color as string
    ).substring(1)}`
    return (
        <div className="player-info">
            <h3>{Color}</h3>
            {!VSComputer && (
                <span className="player-status">{onlineStatus}</span>
            )}
...
const GameInfo: React.FC<GameInfoProps> = (props) => {
    ...
    return (
        <div className="game-info">
            ...
            <p className="game-info_type">
                {gameType}&nbsp;{timing}
            </p>
            <PlayerInfo color={PieceColor.white} />
            <PlayerInfo color={PieceColor.black} />
        </div>
    )
}
