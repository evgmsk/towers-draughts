...
export class BaseMoveResolver {
    ...
    captureTower = (tower: TowerConstructor): TowerConstructor | null => {
        ...
        const white = currentColor === PieceColor.white
        const newTower = Object.assign(Object.assign({}, tower), {
            currentType: TowerType.m,
            wPiecesQuantity: white
                ? (wPiecesQuantity as number) - 1
                : wPiecesQuantity,
            bPiecesQuantity: white
                ? bPiecesQuantity
                : (bPiecesQuantity as number) - 1,
        })
        if (!newTower.bPiecesQuantity && !newTower.wPiecesQuantity) {
            return null as unknown as TowerConstructor
        } else {
            if (white && !newTower.wPiecesQuantity) {
                newTower.currentColor = PieceColor.black
            } else if (!white && !newTower.bPiecesQuantity) {
                newTower.currentColor = PieceColor.white
            }
        }
        return newTower
    }
...
export class MoveResolveCommons extends BaseMoveResolver {
    ...
    makeFreeMove(from: string, to: string, _towers: TowersMap): TowersMap {
        ...
        tower.currentType = this.checkTowerTypeChanging(
            to,
            tower.currentColor,
            tower.currentType
        )
    ...
    makeDraughtMandatoryMove = (move: StepMProps): TowersMap => {
        ...
        const nextMoveProps = {
            ...
            startPosition: this.makeDraughtMandatoryMoveStep(
                firstStepProps,
                true
            ),
    ...
    makeMandatoryMoveStep = (move: StepMProps, last = false): TowersMap => {
        ...
        if (this.GV === 'towers') {
            if (tower.currentColor === PieceColor.white) {
...
export class KingMandatoryMoveResolver extends MoveResolveCommons {
    lookForKingMoves(key: string, towers: TowersMap): ITotalMoves {
        ...
        for (let dir of neighborKeys) {
            ...
            if (
                diagonal.length < 3 ||
                (diagonal.length === 3 && towers[diagonal[2].boardKey])
            ) {
            ...
            if (isDev()) {
                console.warn('first diagonal', moves.mandatory)
            }
    ...
    lookForKingMandatoryMoves(props: FullMRResult[]): FullMRResult[] {
        ...
        if (this.separateCompleted(completed.get()).toCheck.length) {
            console.error('incorrect result')
        }
        return completed.get()
    }

    checkFirstKingDiagonal(
        dir: string,
        diag: BoardCell[],
        towers: TowersMap
    ): ITotalMoves {
        ...
        let mandatory = [] as FullMRResult[],
            free = [] as FreeMRResult[],
            i = 2,
            cellKey = diag[1].boardKey
        while (i < diag.length) {
            ...
            if (prevTower && prevTower?.currentColor !== color && !nextTower) {
                mandatory = mandatory.length
                    ? this.addKingMandatoryStepToResult(
                          mandatory,
                          nextCellKey,
                          dir,
                          cellKey
                      )
                    : this.addFirstMandatoryStep(
                          firstMoveProps,
                          dir,
                          cellKey,
                          nextCellKey
            ...
            if (!prevTower && !nextTower) {
                if (!mandatory.length) {
                    free.push({
                        move: [startCellKey, nextCellKey],
                        startPosition: towers,
                    })
                } else {
                    mandatory = this.addKingMandatoryStepToResult(
                        mandatory,
                        nextCellKey,
                        dir
                    )
    ...
    looKForNewBornKingMoves(props: FullMRResult) {
        ...
        const dir = Object.keys(neighbors).filter(
            (k) => k !== oppositeDirection(lastStepDirection)
        )[0]
        const diag = this.getDiagonal(dir, cellKey)
        const moves = this.separateCompleted(
            this.checkKingDiagonal(diag, props, dir)
        )
    ...
    lookForKingMandatoryDiagonals(
        props: FullMRResult,
        excludeDirection = true
    ): FullMRResult[] {
        const minLength = props.move.length
        ...
        for (let dir of neighborKeys) {
            if (
                dir === oppositeDirection(lastStepDirection!) ||
                (excludeDirection && dir === lastStepDirection)
            ) {
                continue
            }
            const diag = this.getDiagonal(dir, cellKey)
            ...
            if (moves[moves.length - 1].move.length > minLength) {
                extraMove = true
            }
            result.add(moves)
        }
    ...
    separateCompleted(moves: FullMRResult[]): {
        completed: FullMRResult[]
        toCheck: FullMRResult[]
    } {
        return moves.reduce(
            (acc, m) => {
                if (m.move) {
        ...
        while (i < diag.length) {
            ...
            if (
                ...
                return result.length
                    ? result
                    : this.checkIfMoveValidlyCompleted(prevMove)
            }
            if (prevTower && prevTower?.currentColor !== color && !tower) {
                ...
                result = this.addKingMandatoryStepToResult(
                    result,
                    nextCelKey,
                    dir
                )
    ...
    addKingMandatoryStepToResult(
        ...
        if (takenPiece) {
            result = this.addKingStepWithTaken(
            ...
            result = this.addKingStepWithoutTaken(
                this.softSortMoves(moves),
                nextStep,
                dir
            )
        }
    ...
    addKingStepWithoutTaken(
        sortedMoves: FullMRResult[],
        nextStep: string,
        direction: string
    ): FullMRResult[] {
    ...
    addKingStepWithTaken(
        ...
        maxLengthMoves.forEach((m) => {
            ...
            restMoves.push({
                move,
                endPosition: this.makeMandatoryMoveStep(
                    propsToNextPosition,
                    true
                ),
...
export class MovesResolver extends KingMandatoryMoveResolver {
    ...
    getPositionMoves(towers: TowersMap, pieceOrder: PieceColor): ITotalMoves {
        ...
        for (let key of towerKeys) {
            ...
            const { mandatory: mm = [], free: fm = [] } = this.getTowerMove(
                towers,
                key
            )
    ...
    lookForTotalMoves(towers: TowersMap, color: PieceColor): ITotalMoves {
        ...
        for (let key of towerKeys) {
            ...
            mandatory = moves.mandatory?.length
                ? mandatory.concat(moves.mandatory)
                : mandatory
    ...
    lookForManMoves(key: string, towers: TowersMap): ITotalMoves {
        ...
        for (let dir of directions) {
            ...
            if (
                !neighborTower &&
                this.noManMoveRestrictions(color, key, neighborCellKey)
            ) {
                free.push({
                    move: [key, neighborCellKey],
                    startPosition: towers,
                })
            }
            if (neighborTower && neighborTower.currentColor !== color) {
                ...
                if (nextCellKey && !towers[nextCellKey]) {
                    ...
                    const _towers =
                        this.makeMandatoryMoveStep(propsToNextPosition)
    ...
    lookForManMandatoryDirections(props: FullMRResult): FullMRResult[] {
        ...
        if (tower.currentType === TowerType.k) {
            return this.looKForNewBornKingMoves({
                ...props,
                minLength: props.move.length,
            })
        }
        ...
        for (let dir of neighborKeys) {
            ...
            if (
                nextTower &&
                nextTower.currentColor !== color &&
                !takenPieces?.includes(neighborCellKey)
            ) {
                const nextCellKey = this.board[neighborCellKey].neighbors[dir]
                if (nextCellKey && !towers![nextCellKey]) {
    ...
    lookForManMandatoryNextStep(props: FullMRResult[]): FullMRResult[] {
        ...
        for (let move of props) {
            ...
            const moves = this.separateCompleted(
                this.lookForManMandatoryDirections(move)
            )
