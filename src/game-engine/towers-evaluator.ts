...
export class Evaluator {
  ...
  towersValue = (tP: number, bP: number, king = false) => {
    const bottomValue = !king ? (bP * 0.7) / tP : (bP * 0.2) / tP;
  ...
  handlePieces = (tower: ICheckerTower) => {
    const lineValue =
      tower.currentColor === PieceColor.w
        ? parseInt(tower.onBoardPosition.slice(1))
        : 9 - parseInt(tower.onBoardPosition.slice(1));
    const piecePositionValue = (8 + lineValue * 0.3) / 8;
  ...
  getPositionValue(
    ...
    const value =
      parseFloat((advInKings + advInMoves + advInTowers).toFixed(3)) || 0.001;
