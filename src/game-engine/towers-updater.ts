...
export class TowersUpdateResolver extends BaseMoveResolver {
    ...
    updateTowersOnMandatoryMoveStep(
        from: string,
        to: string,
        state: IBoard,
        tP: string[],
        ...
        const towers = this.updateTowersAfterMoveAnimation(
            from,
            to,
            state,
            isTowers,
        ...
        if (isTowers) {
            ...
            const takenTower = this.captureTower(
                middlePiece
            ) as TowerConstructor
            if (!takenTower) {
    ...
    updateTowersAfterMoveAnimation(
        from: string,
        to: string,
        board: IBoard,
        wT = false,
    ...
    animateRivalTowerMove(from: string, to: string, state: IBoard) {
        ...
        opponentTower.positionInDOM = this.calcTowerPosition(
            to,
            cellsMap,
            cellSize
        )
    ...
    getOnboardTowers(towers: TowersMap): TowersMap {
    ...
    updateTowersPosition(
        ...
        Object.keys(towers).forEach((key: string) => {
            _towers[key].positionInDOM = this.calcTowerPosition(
                key,
                map,
                cellSize
            )
    ...
    handleUpToMove = (cellKey: string, board: IBoard) => {
        ...
        const fitMoves = moves.filter((m: MMRResult) => {
            const moveToCheck = m.takenPieces
                ? m.move.slice(mms, mms + 2)
                : m.move
        ...
        if (lastStep) {
            ...
            towers = takenPiece
                ? tur.updateTowersOnMandatoryMoveStep(
                      from,
                      cellKey,
                      board,
                      [takenPiece],
                ...
                : tur.updateTowersAfterMoveAnimation(
                      from,
                      cellKey,
                      board,
                      false,
            ...
            towers = tur.updateTowersOnMandatoryMoveStep(from, cellKey, board, [
                takenPiece,
            ])
        }
        return {
    ...
    handleRemovePiece = (key: string, board: IBoard) => {
        ...
        tower.positionInDOM = tur.calcTowerPosition(
            outboardKey,
            cellsMap,
            cellSize
        )
    ...
    handleSettingPieces = (cellKey: string, board: IBoard): Partial<IBoard> => {
        const { cellSize, cellsMap } = board
        const towerTouched = copyObj(board.towerTouched!) as TowerTouched,
            { key } = towerTouched
        const positionInDOM = this.calcTowerPosition(
            cellKey,
            cellsMap,
            cellSize
        )
    ...
    updateCellsAndTowersPosition(
        ...
        const newTowers = this.updateTowersPosition(
            newCellSize,
            towers,
            newCellMap
        )
