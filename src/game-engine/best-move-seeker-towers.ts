...
import {DebutStage, DefaultMaxDepth, DefaultMinDepth} from "../constants/gameConstants";
...
export class BestMoveSeeker implements SeekerProps {
    ...
    setState = (props: SeekerProps) => {
        this.maxDepth = props.maxDepth || (props.game ? DefaultMaxDepth : DefaultMinDepth)
    ...
    sortMovesByValue(branch: Branch): Move[] {
        ...
        branch.moves = moves.sort((a, b) =>
            children[b.move].deepValue.value[pieceOrder] - children[a.move].deepValue.value[pieceOrder])
        return branch.moves
    }
    ...
    diggingToMaxDepth(depth = this.maxDepth) {
        ...
        let nextBestBranch = children[sortedRootMoves[0].move], extraDepth = 0, currentMove = 0
        while(true) {
            movesTree.getNextDepthData(nextBestBranch)
            nextBestBranch = nextBestBranch.children[nextBestBranch.deepValue.move]
            movesTree.getNextDepthData(nextBestBranch)
            nextBestBranch = nextBestBranch.children[nextBestBranch.deepValue.move]
            extraDepth += 2
            if (extraDepth === depth - deepValue.depth) {
                if (deepValue.value[pieceOrder] < nextBestBranch.deepValue.value[pieceOrder]) {
                    break
    ...
    getMoveFromRootMoves(): Move {
        ...
        console.error('move not found', movesTree.getRoot())
        return {} as Move
    }
